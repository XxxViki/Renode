...
## 调试/仿真提示
- 推荐使用 Renode 仿真：在 `renode` 目录运行脚本即可启动虚拟板卡，避免实体硬件依赖。
...
## VSCode 集成
- `.vscode/tasks.json`：提供构建/烧录/仿真的任务入口，可直接在 VSCode 终端运行。
- `.vscode/launch.json`：预设 GDB 远程调试，目标指向 Renode `3333` 端口，直接 F5 连接即可。
